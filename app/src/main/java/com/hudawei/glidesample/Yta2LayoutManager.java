...
public class Yta2LayoutManager extends RecyclerView.LayoutManager {
    ...
    int scrollBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        int preViewTop = INVALID_SCROLLEDY;
        /**
         * 修改各个Item的top和bottom重新layout
         */
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (preViewTop == INVALID_SCROLLEDY) {
                preViewTop = params.height + top;
            }
            layoutChild(child, top, preViewTop);
            preViewTop = top;
            Log.e("scrollBy", params.getViewAdapterPosition() + " View top:" + child.getTop() + "\t bottom : " + child.getBottom() + "\theight : " + getHeight());
        }
        ...
        /**
         * 添加View
         * 向上滑动，如果第一个View的bottom大于recyclerView的时候，则添加新View，并且设置mScrolledY值为0
        ...
        Log.e("recycler", "startIndex : " + startIndex + "\tendIndex : " + endIndex + "\tdy:"+dy);
        ...
        /*if (downScroll) {
            ...
            YtaLayoutParams params = (YtaLayoutParams) view.getLayoutParams();
            int position = params.getViewAdapterPosition();
            if (view.getBottom() > 0 && position - 1 >= 0) {
                View newView = recycler.getViewForPosition(position - 1);
                YtaLayoutParams newParams = (YtaLayoutParams) newView.getLayoutParams();
...
//        addView(view);
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = 0; i < MAX_SHOW_COUNT; i++) {
            ...
            if (i == 0) {
                preTop = top = maxTop = getHeight() - view.getMeasuredHeight();
                minScrolledY = getScrolledYByTop(maxTop);
                params.mScrolledY = minScrolledY;
                bottom = getHeight();
            } else {
                params.mScrolledY = (int) (minScrolledY + (getHeight() - minScrolledY) * 1.0f / (MAX_SHOW_COUNT - 1) * i);
                top = getTopByScrolling(view, 0);
                bottom = preTop;
                preTop = top;
            }
